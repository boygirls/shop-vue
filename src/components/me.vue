    ...
    <van-row class="col-me">
      ...
      <van-col span="24" class="headerImg">
        <div class="me-img">
          ...
          <img
            src="../../static/images/icon/user.svg"
            name="adapter"
            v-if="userid != 0"
            @click.stop="redirect('#')"
          />
          <img src="" name="adapter" v-else />
        </div>

        <div class="me-img-t">
          <h5 @click="redirects('/login')" v-if="userid == 0">
            {{ login[0] }}
          </h5>
          <h5 v-else>{{ userid.username }}</h5>
        </div>
      ...
      <!-- 我的订单 -->
    <van-button type="primary" @click="tips()"  block plain>我的订单</van-button>
      <!-- 弹出层 -->
      <van-popup v-model="shows" position="top" :overlay="true">
        ...
        >
          ...
          <div v-if="contact" style="height:300px;">
            ...
            <div style="fontSize:12px;color:#A9A9A9;marginTop:20px;">
...
export default {
  ...
  methods: {
    ...
    tips(){
      let path = "/Logistics/"+ this.userid.id
