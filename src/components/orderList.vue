<template>
  ...
  <div id="app">
    <van-loading size="80px" circular v-if="vanload" color="red" class="vanloads" vertical
      >支付中...</van-loading
    >
    <van-sku
      v-model="orderShow"
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      price: state => state.home.addItem.price,
      num: state => state.home.addItem.num,
      userid: state => state.home.userid.id,
    }),
    ...mapGetters(["bc_notshow"])
  },
  methods: {
    ...mapActions(["orderNum", "addGoods", "orderShows"]),
    search_shows() {
      this.$router.back(-1);
    },
    //商品预览
    ...
    onBuyClicked() {
      ...
      //{"orderJson":"[{num:3,itemId:856645},{num:2,itemId:605616}]","userId":48}
      //弹出确认按钮
      Dialog.confirm({
        message: "确认支付吗"
      }).then(() => {
        ...
        // on confirm
        //获取购买数量
        let num = this.quotaUsed;
        let itemId = this.addItem.price;
        this.axios
          .post("http://wyj335.xyz:8088/wechatstore-web/order/ZWH_saveOrder", {
            userId: userid,
            orderJson: [
              {
                num: num,
    ...
    addquotauseds(Event) {
      console.log(Event);
      this.quotaUsed = Event;
    },
    onAddCartClicked() {
      this.addItem.num = this.quotaUsed;
      //将购买的数量和商品加入购物车
      this.addGoods(this.addItem);
      //获取商品的价格
      console.log("购买的价格" + this.addItem.price);
      //获取商品的购买数量
      console.log("购买数量" + this.quotaUsed);
      this.orderShows();
    }
  },
  watch: {},
